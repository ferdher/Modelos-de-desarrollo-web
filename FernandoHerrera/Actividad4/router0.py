...
@router.get("/rou0/" , status_code= status.HTTP_200_OK)
async def usersclass():
    return (users_list)
   
#***Get con Filtro Path
@router.get("/rou0/{id}", status_code = status.HTTP_200_OK)
...
@router.post("/rou0/",  response_model=User, status_code=status.HTTP_201_CREATED)
...
@router.put("/rou0/", status_code= status.HTTP_202_ACCEPTED)
async def usersclass(user:User):
    ...
    #http://127.0.0.1:8000/usersclass/
        ...
        #***Delete
@router.delete("/rou0/{id}", status_code= status.HTTP_200_OK)
async def usersclass(id:int):
